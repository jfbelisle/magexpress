...
class simple_fv {
	...
	private function validate($type, $val, $index_val, $label, $test=false) {
		...
		if($type == 'min_val' && $index_val !== '') {
			if((float)$index_val < (float)$val) {
				if(!$test) {
					...
					$this->custom_error[$label] = __('minimum value is', 'pc_ml').' '.$val;
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'max_val' && $index_val !== '') {
			if((float)$index_val > (float)$val) {
				if(!$test) {
					#$this->errors['value'][] = $label;
					$this->custom_error[$label] = __('maximum value is', 'pc_ml').' '.$val;
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'min_len') {
			if(strlen($index_val) < (int)$val) {
				if(!$test) {
					...
					$this->custom_error[$label] = __('must be at least of', 'pc_ml').' '.$val.' '.__('characters', 'pc_ml');
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'max_len') {
			if(strlen($index_val) > (int)$val) {
				if(!$test) {
					#$this->errors['lenght'][] = $label;
					$this->custom_error[$label] = __('maximum', 'pc_ml').' '.$val.' '.__('characters allowed', 'pc_ml');
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'right_len') {
			if(strlen($index_val) != (int)$val) {
				if(!$test) {
					#$this->errors['lenght'][] = $label;
					$this->custom_error[$label] = __('must be long', 'pc_ml').' '.$val.' '.__('characters', 'pc_ml');
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'min_array') {
			if(!is_array($index_val) || count($index_val) < $val) {
				if(!$test) {
					...
					$this->custom_error[$label] = __('Select', 'pc_ml').' '.$val.' '.__('options', 'pc_ml');
				}
				return false;	
			}
		}
		...
		if($type == 'max_array' && is_array($index_val)) {
			if(count($index_val) > $val) {
				if(!$test) {
					#$this->errors['array_count'][] = $label;
					$this->custom_error[$label] = __('Maximum', 'pc_ml').' '.$val.' '.__('options allowed', 'pc_ml');
				}
				return false;	
			}
		}
	...
	private function errorTranslate($type, $label_array) {
		...
		switch($type) {
			case 'required' : 
				(count($label_array) > 1) ? $message .= __('are required', 'pc_ml') : $message .= __('is required', 'pc_ml');
				break;
			
			case 'wrong_int' : 
				(count($label_array) > 1) ? $message .= __('are not valid integers', 'pc_ml') : $message .= __('is not a valid integer', 'pc_ml');
				break;
				
			case 'wrong_float' : 
				(count($label_array) > 1) ? $message .= __('are not valid floating numbers', 'pc_ml') : $message .= __('is not a valid floating number', 'pc_ml');
				break;	
				
			case 'wrong_mail' : 
				(count($label_array) > 1) ? $message .= __('are not valid e-mail addresses', 'pc_ml') : $message .= __('is not a valid e-mail address', 'pc_ml');
				break;		
				
			case 'wrong_date' : 
				(count($label_array) > 1) ? $message .= __('are not valid dates', 'pc_ml') : $message .= __('is not a valid date', 'pc_ml');
				break;		
				
			case 'wrong_time' : 
				(count($label_array) > 1) ? $message .= __('are not valid times', 'pc_ml') : $message .= __('is not a valid time', 'pc_ml');
				break;
				
			case 'wrong_url' : 
				(count($label_array) > 1) ? $message .= __('are not valid urls', 'pc_ml') : $message .= __('is not a valid url', 'pc_ml');
				break;	
				
			case 'wrong_hex' : 
				(count($label_array) > 1) ? $message .= __('are not valid hexadecimal colors', 'pc_ml') : $message .= __('is not a valid hexadecimal color', 'pc_ml');
				break;	
			
			case 'wrong_ip' : 
				(count($label_array) > 1) ? $message .= __('are not valid IP addresses', 'pc_ml') : $message .= __('is not a valid IP address', 'pc_ml');
				break;	
				
			case 'wrong_zip' : 
				(count($label_array) > 1) ? $message .= __('are not valid ZIP codes', 'pc_ml') : $message .= __('is not a valid ZIP code', 'pc_ml');
				break;
				
			case 'wrong_tel' : 
				(count($label_array) > 1) ? $message .= __('are not valid telephone numbers', 'pc_ml') : $message .= __('is not a valid telephone number', 'pc_ml');
				break;		
					
			case 'wrong_type' : $message .= __('invalid data inserted', 'pc_ml');
				break;	
				
			case 'allowed' : 
				(count($label_array) > 1) ? $message .= __('values are not between the allowed', 'pc_ml') : $message .= __('value is not between the allowed', 'pc_ml');
				break;
				
			case 'forbidden' : 
				(count($label_array) > 1) ? $message .= __('values are between the forbidden', 'pc_ml') : $message .= __('values is between the forbidden', 'pc_ml');
				break;
				
			case 'not_equal' : $message .= __("the value doesn't match", 'pc_ml'); 
				break;
				
			case 'size' : $message .= __('file size is wrong', 'pc_ml');
				break;						
		}	
